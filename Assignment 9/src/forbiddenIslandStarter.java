...
class ForbiddenIslandWorld extends World {
  ...
  void assignToBoard(ArrayList<ArrayList<Double>> heights) {
    ArrayList<ArrayList<Cell>> grid = this.constructCellGrid(heights);
    connectCells(grid);
    this.pieces = new ArrayList<Target>();
    this.board = new MtList<Cell>();
  ...
  ArrayList<ArrayList<Double>> constructOrderHeights() {
  ...
  ArrayList<ArrayList<Double>> constructRandomHeights() {
    ArrayList<ArrayList<Double>> result = new ArrayList<ArrayList<Double>>();
    for (int x = 0; x <= ForbiddenIslandWorld.ISLAND_SIZE; x += 1) {
      ArrayList<Double> col = new ArrayList<Double>();
      for (int y = 0; y <= ForbiddenIslandWorld.ISLAND_SIZE; y += 1) {
  ...
  ArrayList<ArrayList<Double>> constructTerrain() {
    ArrayList<ArrayList<Double>> result = new ArrayList<ArrayList<Double>>();
  ...
  ArrayList<ArrayList<Cell>> constructCellGrid(ArrayList<ArrayList<Double>> heights) {
  ...
  // returns a cell that is not currently occupied and has a path to a given
  // cell
  ...
  public void onKeyEvent(String ke) {
    if (ke.equals("t")) {
      this.assignToBoard(this.constructTerrain());
      this.waterHeight = 0;
      this.tick = 0;
    }
    if (ke.equals("m")) {
      this.assignToBoard(this.constructOrderHeights());
      this.waterHeight = 0;
      this.tick = 0;
    }
    if (ke.equals("r")) {
      this.assignToBoard(this.constructRandomHeights());
      this.waterHeight = 0;
      this.tick = 0;
    }
...
class ExamplesForbiddenIsland {
  ...
  void init() {
    this.perfectHeights = this.normal.constructOrderHeights();
    this.randomHeights = this.random.constructRandomHeights();
    this.terrainHeights = this.terrain.constructTerrain();
  ...
  void testGeneratePerfectHeights(Tester t) {
    ...
    int x = 0;
    while (x < 10) {
      int randomX = (int) (Math.random() * this.perfectHeights.size());
      int randomY = (int) (Math.random() * this.perfectHeights.get(randomX).size());
  ...
  void testGenerateRandomHeights(Tester t) {
    ...
    int x = 0;
    while (x < 10) {
      int randomX = (int) (Math.random() * this.randomHeights.size());
      int randomY = (int) (Math.random() * this.randomHeights.get(randomX).size());
      t.checkRange(this.randomHeights.get(randomX).get(randomY).intValue(), 0, 33);
      x += 1;
    }
  }
  ...
  void testGenerateCellGrid(Tester t) {
    this.init();
    ArrayList<ArrayList<Cell>> cellGrid = this.normal.constructCellGrid(this.perfectHeights);
    int x = 0;
    while (x < 10) {
      int randomX = (int) (Math.random() * this.randomHeights.size());
      int randomY = (int) (Math.random() * this.randomHeights.get(randomX).size());
  ...
  void testDistanceToCenter(Tester t) {
    ...
    while (x < 10) {
      ...
      t.checkExpect(this.normal.distanceToCenter(randomX, randomY),
          (double) ((Math.abs((ForbiddenIslandWorld.ISLAND_SIZE / 2) - randomX)
              + Math.abs((ForbiddenIslandWorld.ISLAND_SIZE / 2) - randomY))));
      x += 1;
    }
  }
  ...
  void testDrawPlayer(Tester t) {
    ...
    bg2.placeImageXY(new FromFileImage("./src/pilot-icon.png"), p.cell.x * 10 + 5,
        p.cell.y * 10 + 5);
    p.drawPlayer(bg);
  ...
  void testGetHighestCell(Tester t) {
    this.init();
    ArrayList<ArrayList<Cell>> grid = this.normal.constructCellGrid(this.perfectHeights);
    t.checkExpect(this.normal.getHighestCell(grid), grid.get(32).get(32));
  ...
  void testUpdatePieces(Tester t) {
    ...
    ArrayList<ArrayList<Cell>> connected = this.noBoard.constructCellGrid(temp);
  ...
  void testUpdatePlayer(Tester t) {
    ...
    Cell player = this.noBoard.updatePlayer(occupied, connected, occupied.get(0));
