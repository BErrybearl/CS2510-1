...
class ExamplesPermutation {
  ArrayList<Character> alphabet = new ArrayList<Character>(
      Arrays.asList('h', 'w', 'z', 'a', 'm', 'q', 'k', 'l', 'e', 'p', 'v', 'b', 'o', 'g', 'y', 'x',
          'c', 'd', 'r', 'i', 'f', 'j', 'n', 't', 's', 'u'));
  ArrayList<Character> code2 = new ArrayList<Character>(Arrays.asList('h', 'm', 'q', 'k', 'l', 'e',
      'p', 'v', 'b', 'o', 'g', 'x', 'd', 'r', 'i', 'f', 'j', 'n', 's', 'u'));
  ArrayList<Character> code3 = new ArrayList<Character>(Arrays.asList('h', 'm', 'p', 'v', 'b', 'o',
      'g', 'x', 'd', 'r', 'i', 'f', 'j', 'n', 's', 'u'));
  ...
  void testEncode(Tester t) {
    ...
    t.checkExpect(p1.encode("abc"), "hwz");
    t.checkExpect(p1.encode(""), "");
  }
  ...
  void testDecode(Tester t) {
    ...
    t.checkExpect(p1.decode("abc"), "dlq");
    t.checkExpect(p1.decode(""), "");
  }
  ...
  boolean testDecodeHelper(Tester t) {
    return t.checkExpect(p1.decodeHelper(this.encoded1, "abc"), "abcolehl ban")
        && t.checkExpect(p1.decodeHelper("", "abc"), "abc")
        && t.checkExpect(p1.decodeHelper("abc !@# cba", " k ! j"), " k ! jdlq !@# qld");
  }

  // test the EncodeHelper method
  boolean testEncodeHelper(Tester t) {
    return t.checkExpect(p1.encodeHelper(this.encoded1, "abc"), "abcswobw nlk")
        && t.checkExpect(p1.encodeHelper("", "abc"), "abc")
        && t.checkExpect(p1.encodeHelper("abc !@# cba", " k ! j"), " k ! jhwz !@# zwh");
  }
}